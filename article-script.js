// Article data
const articles = {
  article1: {
    category: "SPOTLIGHT",
    title: "Diễn viên Nguyễn Quang Tiến khẳng định “ca sĩ Miho Chu chưa nên dấn thân vào làng điện ảnh",
    content: `
       <p class="time">Chủ nhật, 24/05/2025 | 10:30 (GMT+7)</p>
     <p class = "lead">Trong đoạn phỏng vấn cùng Yeah1, diễn viên nổi tiếng kiêm giảng viên Trường đại học Sân khấu điện ảnh Nguyễn Quang Tiến, bằng con mắt 20 năm trong nghề của mình, đã khẳng định ca sĩ trẻ Miho Chu chưa nên vội vã lấn sân sang làng điện ảnh.  </p>
    <p>Sau thời gian dài vắng bóng trên màn ảnh do bận rộn với dự án sản xuất phim đầu tiên do chính mình làm đạo diễn, diễn viên Quang Tiến đã đồng ý tham gia vào vai diễn “người cha” trong MV mới của ca sĩ Miho Chu. Đây không chỉ là một màn trở lại đáng mong chờ mà còn là lần đầu tiên diễn viên Quang Tiến tham gia vào một dự án âm nhạc.</p>

    <p>Trong dự án lần này, anh đã hợp tác ghi hình cùng ca sĩ trẻ tuổi Miho Chu. Miho Chu vốn là một giọng hát đầy nội lực từng làm mưa làm gió trong làng nhạc Việt khi xuất hiện trong chương trình The Voice Kids năm 2015. Tuy chỉ dừng lại ở vị trí thứ 8 nhưng ca sĩ Miho Chu đã để lại không ít ấn tượng trong lòng khán giả bởi tông giọng lạ cùng gia thế “khủng” - cháu ngoại NSND Tiến Dũng - ca sĩ Phương Thanh cùng hai người bác là ca sĩ Hương Tràm, và ca sĩ Tiến Mạnh. Thế nhưng ca sĩ Miho đã ngụp lặn 10 năm trong nghề và tới tận năm nay, anh mới trở lại với sản phẩm âm nhạc “Playlist” - cũng chính là MV có sự hợp tác của diễn viên Quang Tiến.</p>

    <p>Phỏng vấn diễn viên Quang Tiến về trải nghiệm hợp tác cùng ca sĩ Miho, anh đánh giá Miho là một giọng hát tốt và đầy triển vọng. Anh đã trực tiếp lắng nghe ca khúc của Miho nhiều lần từ bản thu tới hát live ngoài đời, lần nào Miho cũng khiến anh phải tấm tắc khen ngợi vì tài năng ca hát xuất sắc.</p>

    <figure>
      <img src="./asset/bao/1/1.webp" alt="Minimalist lifestyle" />
      <figcaption >hình ảnh từ MV “Playlist” </figcaption>
    </figure>

    <p>Đánh giá về diễn xuất của ca sĩ Miho Chu khi Miho trực tiếp tham gia vào vai trò diễn viên MV, diễn viên Quang Tiến chia sẻ “Miho đã làm khá tốt”. Anh thuật lại rằng trong suốt quá trình ghi hình, ca sĩ Miho đã gặp nhiều khó khăn khi không nắm bắt được cảm xúc nhân vật và không thể kiểm soát hoàn toàn biểu cảm của mình. Anh cùng với ekip đã phải hỗ trợ ca sĩ Miho rất nhiều trong suốt quá trình ghi hình và thật may mắn, Miho là một “học trò” nhanh nhạy, tiếp thu nhanh và rất cầu tiến. Thêm vào đó anh còn khẳng định ca sĩ Miho đang quá tự ti mỗi khi đánh giá về khả năng diễn xuất của chính mình. </p>

    <p>Thế nhưng khi được hỏi “Anh nghĩ Miho có nên lấn sân sang làng điện ảnh không?” thì anh trả lời “Tôi nghĩ thời điểm hiện tại ca sĩ Miho Chu chưa nên dấn thân vào làng điện ảnh. Xu hướng hiện nay đang có rất nhiều bạn ca sĩ trẻ lấn sân sang con đường diễn xuất. Đối với với Miho, mình thấy được khả năng của bạn trong vai trò ca sĩ. Ở giai đoạn đầu mới debut, bạn nên tập trung vào con đường âm nhạc để phát huy hết tài năng vốn có của mình hơn là vội vàng chuyển hướng sang điện ảnh, như thế rất uổng phí.” </p>
    <p style="text-align: right;"><strong>Biên tập: Sally Nguyễn</strong></p>
     <p style="text-align: right;"><strong>Phỏng vấn: Đặng Ngân, Sally Nguyễn</strong></p>
        <div class="article-tags">
      <span class="article-tag">Spotlight</span>
      <span class="article-tag">Sao</span>
      <span class="article-tag">Âm nhạc</span>
    </div>
   
  `
  },

  article2: {
    category: "HẬU TRƯỜNG",
    title: "Phía sau hậu trường, ca sĩ Miho Chu quỳ gối trước đạo diễn Hoàng Long",
    content: `
       <p class="time">Thứ sáu, ngày 30/05/2025 | 20:00 (GMT +7)</p>
       <p class="lead">Ngay trước ngày chính thức ra mắt MV "Playlist", ca sĩ trẻ Miho Chu đã có hành động quỳ gối trước đạo diễn Hoàng Long với sự chứng kiến của nhiều nhân vật quan trọng ekip sản xuất MV.</p>

    <p>Sau 5 tiếng tổng duyệt họp báo đầy căng thẳng, một số thành viên của PRism Production cùng với ca sĩ, diễn viên của MV "Playlist" đã tụ họp với nhau để giải trí bằng trò chơi "Tiến lên" với hình phạt quỳ. Sau 2 lượt chơi, ca sĩ Miho Chu đã phải "quỳ gối" khuất phục trước đạo diễn Hoàng Long.</p>

     <figure>
      <img src="./asset/bao/2/3.webp" alt="Ca sĩ Miho Chu quỳ gối" />
      <figcaption>Ca sĩ Miho Chu "quỳ gối" trước đạo diễn Hoàng Long</figcaption>
    </figure>


    <p>Thế nhưng chỉ thêm vài ván so tài thì vận đen đã chuyển lên người đạo diễn. Lần này cả nhóm sản xuất lại được dịp trêu chọc nam đạo diễn trẻ tuổi.</p>

     <figure>
      <img src="./asset/bao/2/2.webp" alt="Đạo diễn Hoàng Long bị phạt" />
      <figcaption>Cả nhóm vui vẻ trêu chọc đạo diễn Hoàng Long khi anh bị phạt quỳ</figcaption>
    </figure>


    <figure>
      <img src="./asset/bao/2/6.webp" alt="Tổng biên kịch Chu Đào Ngọc Anh" />
      <figcaption>Tổng biên kịch Chu Đào Ngọc Anh vẫn cười tươi khi bị phạt</figcaption>
    </figure>

    <figure>
      <img src="./asset/bao/2/7.webp" alt="Ca sĩ Miho Chu đắc ý" />
      <figcaption>Ca sĩ Miho Chu "đắc ý" khoe vận đỏ với diễn viên Thanh Thanh</figcaption>
    </figure>

    <p>Bầu không khí tại hậu trường rất ấm áp và gần gũi. Sau thời gian dài đồng hành làm việc trong dự án MV "Playlist", các thành viên của đội ngũ sản xuất đã coi nhau là bạn bè tốt và thoải mái cười đùa. Các nghệ sĩ cũng chia sẻ họ thường xuyên ra ngoài ăn uống hoặc đi chơi mặc cho những khoảng cách về tuổi tác. Họ cũng bày tỏ mong muốn sẽ được tiếp tục hợp tác trong nhiều dự án khác.</p>

    <p>Ngoài thời gian cả nhóm cùng chơi đùa, rất nhiều khoảnh khắc bắt gặp nhóm nghệ sĩ - đội ngũ sản xuất chia thành cặp ngồi tâm sự gần gũi với nhau về các câu chuyện làm nghề hoặc chuyện đời sống hằng ngày.</p>

    <figure>
      <img src="./asset/bao/2/1.webp" alt="Diễn viên Quang Tiến plank" />
      <figcaption>Diễn viên Quang Tiến khoe tài plank 1 phút trước đạo diễn Hoàng Long</figcaption>
    </figure>

     <figure>
      <img src="./asset/bao/2/4.webp" alt="Trưởng BTC Phương Nga tạo dáng" />
      <figcaption>Trưởng BTC Phương Nga tạo dáng nhí nhảnh cùng diễn viên chính và ca sĩ</figcaption>
    </figure>
     <figure>
      <img src="./asset/bao/2/5.webp" alt="Giám đốc nghệ thuật và đạo diễn" />
      <figcaption>Giám đốc nghệ thuật Đoan Trang cùng đạo diễn Hoàng Long tâm sự thân thiết</figcaption>
    </figure>


    <p style="text-align: right;"><strong>Biên tập: Sally Nguyễn</strong></p>
    <div class="article-tags">
      <span class="article-tag">Hậu trường</span>
      <span class="article-tag">Sao</span>
    </div>
    `
  },
  article3: {
    image: "./asset/bao/3/thumbmail.webp",
    category: "Âm nhạc",
    title: `Màn diễn bùng nổ được tái hiện ngay trong 
buổi họp báo ra mắt MV “Playlist”
`,
    content: `
       <p class="time">Thứ bảy, 31/5/2025 | 13:20 (GMT+7)</p>
       <p class="lead">Sau bao ngày tháng chờ đợi, buổi họp báo ra mắt MV "Playlist", đánh dấu sự trở lại của nghệ sĩ trẻ Miho Chu đã chính thức được tổ chức vào 10 giờ sáng ngày 31/5/2025 tại Thai Room, Học viện Ngoại giao với sự tham gia của ekip sản xuất, đạo diễn Hoàng Long cùng diễn viên Thanh Thanh và diễn viên Quang Tiến - hai diễn viên chính trong MV lần này.</p>

    <p>Trong buổi họp báo ra mắt MV, các quý vị khách mời đã lần đầu được thưởng thức những thước phim đầy nghệ thuật. Được biết, ekip sản xuất đã phải mất hàng tháng trời làm việc không ngừng nghỉ để hoàn thiện sản phẩm ra mắt của ca sĩ Miho Chu.</p>

   

<div class="image-row">
  <figure>
    <img src="./asset/bao/3/1.webp" alt="Hình ảnh từ MV Playlist" />
  </figure>
  <figure>
    <img src="./asset/bao/3/2.webp" alt="Hình ảnh từ MV Playlist" />
  </figure>
  <figcaption>Hình ảnh từ MV "Playlist"</figcaption>
</div>

    <p>Bên cạnh sự chỉn chu trong sản xuất MV, đội ngũ PRism Production cũng chuẩn bị rất kỹ lưỡng cho ngày ra mắt chính thức. Buổi họp báo được trang trí cùng chủ đề với MV, xoay quanh đĩa cassette và hạc giấy.</p>
 <figure>
      <img src="./asset/bao/3/3.webp" alt="Hình ảnh tại buổi họp báo" />
      <figcaption>Hình ảnh tại buổi họp báo</figcaption>
    </figure>
    <p>Sau khi trả lời những câu hỏi của phía nhà báo, bộ ba diễn viên chính xuất hiện trong MV lần này: ca sĩ Miho Chu (trong vai người con), diễn viên Trần Thị Thanh Thanh (trong vai người mẹ) và diễn viên Nguyễn Quang Tiến (trong vai người bố) đã có một màn diễn xuất trực tiếp ngay trên sân khấu của buổi họp báo. Bộ ba đã diễn lại cảnh quay nổi bật nhất cũng là cảnh quay để lại ấn tượng nhất trong kí ức của ba diễn viên: cảnh nhân vật con trình bày mong muốn được theo đuổi sự nghiệp âm nhạc gây ra mâu thuẫn lớn trong cả gia đình. Đây là cảnh quay đã lấy đi nhiều nước mắt của cả ba diễn viên khi ghi hình cho MV bởi sự căng thẳng và chân thật trong từng lời thoại cũng như sự đồng cảm của họ đối với câu chuyện của nhân vật.</p>

   

   <div class="video-wrapper">
  <div class="video-container">
    <iframe src="https://drive.google.com/file/d/1Jx-g0DmBFCh1LIoWFVqoD2c004zWemfw/preview" frameborder="0"  allow="autoplay" allowfullscreen></iframe>
  </div>
  <figcaption>Khung cảnh trong MV được tái hiện ngay trong buổi ra mắt họp báo</figcaption>
</div>

    <p style="text-align: right;"><strong>Biên tập: Sally Nguyễn</strong></p>
    <div class="article-tags">
      <span class="article-tag">Spotlight</span>
      <span class="article-tag">Âm nhạc</span>
      <span class="article-tag">Sao</span>
    </div>
    `
  },
  article4: {
    category: "SPOTLIGHT",
    title: "Gia đình nghệ sĩ Miho Chu không xuất hiện tại họp báo ra mắt MV đầu tay của con trai",
    content: `
      <p class="time">Thứ bảy, 31/5/2025 | 15:00 (GMT+7)</p>
      <p class="lead">Ngày 31/05/2025, buổi ra mắt MV "Playlist" của ca sĩ Miho Chu đã chính thức diễn ra tại Thai room, Học viện Ngoại giao với sự tham gia của diễn viên Nguyễn Quang Tiến, Trần Thị Thanh Thanh, đạo diễn Hoàng Long và nhiều đơn vị báo chí. Tuy nhiên, phía gia đình của nghệ sĩ Miho Chu đã vắng bóng trong buổi họp báo ra mắt sản phẩm MV đầu tay của con trai mình.</p>

      <p>Ca sĩ Chu Minh Hoàng (hay còn biết tới với nghệ danh Miho Chu), đã bất ngờ ra mắt ca khúc "Playlist" sau 10 năm vắng bóng trong làng âm nhạc Việt. Đây là dấu mốc quan trọng của trong cuộc đời của chàng nghệ sĩ trẻ và đặc biệt hơn, buổi họp báo ra mắt MV "Playlist" không đơn thuần là một buổi giới thiệu sản phẩm âm nhạc mà còn là lần xuất hiện chính thức của ca sĩ Miho Chu trước công chúng kể từ năm 2015. Thế nhưng, trong buổi họp báo, phía gia đình nghệ sĩ Miho Chu, bao gồm cả bố mẹ của anh - những giảng viên kì cựu tại Học viện Âm nhạc Việt Nam hay ông bà ngoại (NSND Tiến Dũng - ca sĩ Phương Thanh) và hai người bác - ca sĩ Hương Tràm, ca sĩ Tiến Mạnh đều không xuất hiện.</p>

      <figure>
        <img src="./asset/bao/4/1.webp" alt="Ca sĩ Miho Chu lo lắng trong buổi họp báo" />
        <figcaption>Ca sĩ Miho Chu lo lắng trong buổi họp báo</figcaption>
      </figure>

      <p>Trong buổi họp báo, nghệ sĩ Miho Chu giải thích rằng các thành viên trong gia đình anh đều có lịch trình cá nhân nên không thể góp mặt trong ngày ra mắt. Anh cũng chia sẻ, hiện tại gia đình rất tự hào về anh và sẵn sàng ủng hộ anh trên con đường âm nhạc. Anh cũng bật mí rằng chính mẹ anh là người ngỏ lời mời đạo diễn Hoàng Long thực hiện các cảnh quay trong MV "Playlist".</p>

      <p>Khi được hỏi về vấn đề tận dụng những nguồn lực từ gia đình để dấn thân vào con đường âm nhạc, ca sĩ Miho Chu đã khẳng định "Mình không ngần ngại nhận sự giúp đỡ của gia đình. Gia đình là nền tảng vững chắc cho mình và mình rất trân trọng điều đó, Tuy nhiên, mình cũng luôn phải nỗ lực hết mình để xứng đáng với những cơ hội cũng như xứng đáng với kỳ vọng mà cả gia đình đặt cho mình."</p>

    
        <figure class = "a">
          <img src="./asset/bao/4/2.webp" alt="Hình ảnh của ca sĩ Miho Chu trong buổi họp báo" />
        </figure>
        <figure class = "b">
          <img src="./asset/bao/4/3.webp" alt="Hình ảnh của ca sĩ Miho Chu trong buổi họp báo" />
           <figcaption>Một số hình ảnh khác của ca sĩ Miho Chu trong buổi họp báo</figcaption>
        </figure>
       
      

      <p style="text-align: right;"><strong>Biên tập: Sally Nguyễn</strong></p>
      <div class="article-tags">
        <span class="article-tag">Spotlight</span>
        <span class="article-tag">Sao</span>
        <span class="article-tag">Âm nhạc</span>
      </div>
    `
  },
  article5: {

    category: "PHỎNG VẤN",
    title: `Gặp Long Đỗ – đạo diễn trẻ đứng sau MV "Playlist" của Miho Chu"`,
    content: `
      <p class="time">Thứ ba, 26/05/2025 | 09:32 (GMT+7)</p>
      <p class="lead">Mới 19 tuổi, Long Đỗ đã kịp gây chú ý với tư cách đạo diễn chính MV "Playlist" – sản phẩm đánh dấu sự kết hợp cùng ca sĩ trẻ Miho Chu. Không theo đuổi phong cách kịch tính, MV này lại ghi điểm nhờ cảm xúc chân thật và cách kể chuyện bằng hình ảnh tối giản nhưng nhiều chiều sâu. Đại diện cho thế hệ nghệ sĩ Gen Z, Long Đỗ chọn làm nghệ thuật theo một con đường riêng: kiên định, kỹ tính và độc lập.</p>

      <p>Chúng tôi đã có cuộc trò chuyện cùng Long về hành trình làm MV đầu tiên, quan điểm thẩm mỹ cá nhân và cách một đạo diễn trẻ đang đi tìm tiếng nói riêng trong thế giới hình ảnh.</p>

      <figure>
        <img src="./asset/bao/5/1.webp" alt="Đạo diễn Long Đỗ trong quá trình quay MV Playlist" />
        <figcaption>Đạo diễn Long Đỗ trong quá trình quay MV "Playlist"</figcaption>
      </figure>

      <p class="quote highlight">Áp lực lớn nhất đến từ chính bản thân mình</p>

      <p class="interview-question">- &nbsp &nbsp Bạn và ca sĩ Miho Chu đều là những người nghệ sĩ/tài năng trẻ của Việt Nam, vậy các bạn có áp lực gì đối với tác phẩm này không? Bạn có kỳ vọng vào tác phẩm này?</p>
      <p><strong>Đạo diễn Long Đỗ</strong>: Tác phẩm lần này để hoàn thiện đã phải trải qua rất nhiều lần sửa đổi, qua rất nhiều bản nháp để có được kịch bản cuối cùng, mình cũng phản hồi lại rất nhiều. Khi làm việc với các bạn biên kịch, có những ý tưởng chủ quan rất khó để diễn đạt một cách chính xác ngay từ đầu, nên sẽ phải qua nhiều lần trao đổi và sửa đổi. Nhưng cuối cùng chúng mình cũng đã tạo nên một kịch bản khá hay và mình mong khán giả sẽ đón nhận nó.</p>
      <p>Áp lực chắc chắn là có, nhưng áp lực lớn nhất thì có lẽ là áp lực đến từ chính bản thân mình. Tất nhiên áp lực từ khán giả cũng là một phần nữa. Mình luôn muốn sản phẩm của mình được trau chuốt. Mình mong là khi các bạn xem những sản phẩm sắp tới đây thì các bạn có thể hiểu tại sao.</p>

      <p class="interview-question">- &nbsp &nbsp Vậy trong suốt quá trình quay dựng của bạn, bạn có tác phẩm nào tâm đắc nhất?</p>
      <p><strong>Đạo diễn Long Đỗ</strong>: Mình thấy để đặt vào bàn cân với những sản phẩm khác thì cái này (MV "Playlist") có đầu ra toàn diện nhất, nhưng mà tất nhiên là mình vẫn chưa ưng rồi. Mình luôn muốn mình phát triển. Tác phẩm này dù đã hoàn thiện nhưng sau này mình xem lại thì sẽ có những hạt sạn cần khắc phục, kể cả bây giờ mình xem lại cũng đã thấy sạn. Thế nên để nói chung thì đây là sản phẩm hoàn chỉnh nhất của mình từ trước đến giờ, còn sau này thì có thể thay đổi.</p>

      <figure>
        <img src="./asset/bao/5/2.webp" alt="Đạo diễn Long Đỗ trong quá trình quay MV Playlist" />
        <figcaption>Đạo diễn Long Đỗ trong quá trình quay MV "Playlist"</figcaption>
      </figure>

      <p class="quote highlight">Nghệ thuật là một phương tiện để truyền tải câu chuyện</p>

      <p class="interview-question">- &nbsp &nbsp Tiêu chí đánh giá một tác phẩm tốt của bạn là gì?</p>
      <p><strong>Đạo diễn Long Đỗ</strong>: Một tác phẩm tốt của mình là một tác phẩm mà nó bày tỏ được cái tôi của mình một cách toàn diện nhất, đầy đủ nhất. Nó có chứa đựng những cái hàm ý trong đấy. Và tất nhiên phải đẹp nữa.</p>

      <p class="interview-question">- &nbsp &nbsp Khi bắt tay vào sản xuất, đâu là yếu tố bạn ưu tiên hàng đầu: câu chuyện, hình ảnh, cảm xúc hay thị hiếu khán giả?</p>
      <p>Còn về thị hiếu khán giả thì mình nghĩ là tùy. Thị hiếu đôi khi mang tính khách quan, nhưng nghệ thuật lại là thứ phụ thuộc vào cảm nhận của mỗi người. Một bộ phim có thể người này thấy hay, người kia lại thấy dở. Vậy nên trong quá trình làm việc, mình không quá để tâm người khác nghĩ gì hay khán giả sẽ đón nhận ra sao. Mình quan tâm nhất là bản thân mình nghĩ gì. Với mình, đó mới là nghệ thuật.</p>

      <p class="quote highlight">Kỹ tính là một thuận lợi</p>

     <p class="interview-question">- &nbsp &nbsp Là một đạo diễn trẻ nhưng có vẻ anh là một người rất kỹ tính (có thể coi là khó tính) trong công việc. Anh có xem đó là cản trở hay thuận lợi trong công việc của mình không?</p>
      <p><strong>Đạo diễn Long Đỗ</strong>: Mình coi đó là một thuận lợi. Để có thể sản xuất được một sản phẩm mang dấu ấn cá nhân, thì sự kỹ tính và có phần bảo thủ là khá quan trọng. Vì mỗi sản phẩm, như mình đã nói, là một câu chuyện mà mình muốn kể – và đó là quan điểm cá nhân của mình. Trong quá trình làm việc với MV này, mình rất kỹ tính trong việc chọn lọc những nhận xét để tiếp thu, và cũng khá bảo thủ với chính quan điểm của mình. Mình chỉ xem xét những góp ý nào thực sự giúp ích cho câu chuyện mà mình đang kể.</p>

      <p class="interview-question">- &nbsp &nbsp Trong quá trình làm MV "Playlist", ngoài việc làm việc với các diễn viên thì bạn còn phải làm việc với ekip của mình. Bạn có đánh giá ntn về quá trình làm việc với đội ngũ ekip này và bạn có cảm thấy vất vả trong quá trình làm việc không?</p>
      <p><strong>Đạo diễn Long Đỗ</strong>: Vất vả thì ngày nào cũng vất vả thôi, mỗi thứ lại có kiểu vất vả riêng. Làm phim là một quá trình tiêu tốn rất nhiều năng lượng, cả về thể chất lẫn trí tuệ. Ngoài dàn diễn viên, mình khá hài lòng với đội ngũ làm việc lần này. Thực ra, mình có 1–2 người quen trong ekip – họ rất hiểu việc, biết rõ mình cần làm gì về mặt kỹ thuật để quá trình quay và set-up diễn ra nhanh chóng, tiết kiệm thời gian. Còn lại, những thành viên khác trong ekip cũng đã làm hết sức, cố gắng và nỗ lực rất nhiều trong suốt quá trình quay. May mắn là mọi thứ diễn ra khá trơn tru, không có khúc mắc nào về mặt nhân sự cả.</p>

      <p class="quote highlight">"Cái gu, cái hình ảnh hướng tới đã ở trong tiềm thức"</p>

      <p class="interview-question">- &nbsp &nbspCó đạo diễn, nhà quay phim, hoặc họa sĩ nào mà anh xem là nguồn cảm hứng lâu dài cho các sản phẩm của mình không?</p>
      <p><strong>Đạo diễn Long Đỗ</strong>: Mình thực ra không có một ai cụ thể mà mình chịu ảnh hưởng một cách rõ rệt. Các sản phẩm của mình thường theo trường phái nghệ thuật phương Tây, vì mình xem rất nhiều phim phương Tây. Có một đạo diễn mà mình rất thích xem, đó là Quentin Tarantino. Nguồn cảm hứng này thực ra lại khá trái ngược với những gì mình đang làm – đặc biệt là với MV "Playlist". Tarantino thường kể chuyện theo lối phi tuyến tính, không đi theo mạch thời gian, và cách ông nối các trục cảnh quay rất có chủ đích. Mình cũng muốn sản phẩm của mình như vậy – mọi thứ xuất hiện trong khuôn hình, từ bố cục đến cách sắp xếp, đều phải có lý do nhất định, chứ không phải cứ thấy đẹp là đưa vào.</p>
      <p>Về mặt hình ảnh thì mình xem rất nhiều nên bị ảnh hưởng bởi rất nhiều tác phẩm. Sẽ có một cái gu mà mình thường sẽ hướng tới, nhưng mình sẽ không thể nói rõ được vì nó ở trong tiềm thức của mình. Chỉ là mình sẽ tự nhiên bị thu hút và hướng tới một kiểu hình ảnh nhất định.</p>

      <figure class = "abc">
        <img src="./asset/bao/5/3.webp" alt="Hình ảnh từ góc máy quay của đạo diễn Long Đỗ cho MV Playlist" />
        <figcaption > Hình ảnh từ góc máy quay của đạo diễn Long Đỗ cho MV "Playlist"</figcaption>
      </figure>

      <p class="quote highlight">"Hãy cứ cố gắng, hãy cứ tiếp tục theo đuổi"</p>

      <p class="interview-question"> - &nbsp &nbsp Giữa một MV thiên về nghệ thuật và một MV "viral", anh chọn gì? Hay có cách nào để dung hòa cả hai?</p>
      <p><strong>Đạo diễn Long Đỗ</strong>: Tất nhiên là mình sẽ chọn một MV thiên về nghệ thuật rồi. Nhưng những bộ phim hay MV mà mình thấy hay nhất thì chưa chắc người khác cũng nghĩ vậy – đa phần là thế. Nói chung, cảm nhận là tùy từng người, nhưng cá nhân mình vẫn luôn thiên về MV nghệ thuật hơn là chạy theo sự viral.</p>

      <p>Qua câu chuyện trong MV, bạn có lời nào muốn gửi gắm đối với thế hệ trẻ Việt Nam theo đuổi niềm đam mê nghệ thuật nói chung không?</p>
      <p><strong>Đạo diễn Long Đỗ</strong>: Vì mình là người trẻ nên mình cũng phải nghe theo câu này, có thể các bạn đã nghe nhiều rồi nhưng mà: hãy cứ cố gắng, hãy cứ tiếp tục theo đuổi.</p>

      <div class="video-wrapper">
        <div class="video-container">
          <iframe src="https://drive.google.com/file/d/1PfXuy3LfC6q3JBpH1h4tJ2nEt1teO2jM/preview" frameborder="0" allow="autoplay" allowfullscreen></iframe>
        </div>
        <figcaption>Đạo diễn Long Đỗ đang làm việc trong hậu trường MV "Playlist"</figcaption>
      </div>

      <p style="text-align: right;"><strong>Biên tập: Đặng Ngân</strong></p>
      <p style="text-align: right;"><strong>Phỏng vấn: Đặng Ngân, Sally Nguyễn</strong></p>
      <div class="article-tags">
        <span class="article-tag">Phỏng vấn</span>
        <span class="article-tag">Hậu Trường</span>
      </div>
    `
  },
  // ...existing code...

  article6: {
    category: "SPOTLIGHT",
    title: `"MV "Playlist": Khi hai tài năng gen Z bắt tay"`,
    content: `
      <p class="time">Chủ Nhật, 01/06/2025 | 09:00 (GMT+7)</p>
      <p class="lead">Giữa thị trường âm nhạc trẻ đang chạy theo những cú "hit nhanh – viral mạnh", ca sĩ Miho Chu và đạo diễn Long Đỗ đã cùng bắt tay tạo ra MV "Playlist" với lối đi đầy cảm xúc và chiều sâu. MV "Playlist" đánh dấu màn trở lại âm nhạc của Miho Chu, đồng thời là sự kết hợp nghệ thuật chỉn chu với đạo diễn trẻ Long Đỗ, mang đến một sản phẩm độc lập vừa chân thật vừa khác biệt.</p>

      <p>Ca sĩ Miho Chu (tên thật: Chu Minh Hoàng) từng gây chú ý với khán giả khi lọt vào Top 8 chương trình The Voice Kids 2015, tham gia nhiều sân chơi âm nhạc như Sao Mai xứ Nghệ hay Anh trai toàn năng. Thế nhưng, sau những thành công đầu đời, Miho đã chọn cách tạm gác lại ánh đèn sân khấu để lắng nghe chính mình và nuôi dưỡng đam mê theo cách riêng.</p>

      <p>Trở lại sau thời gian dài im ắng, Miho ra mắt MV "Playlist" – một bản indie-pop mang màu sắc hoài niệm. Với giai điệu nhẹ nhàng và lời ca chân thành như dòng nhật ký cảm xúc, đây không chỉ là sản phẩm đánh dấu sự trở lại, mà còn mở ra một chương mới sâu sắc, trưởng thành hơn trong hành trình âm nhạc của anh.</p>

      <p>Nhưng chỉ âm nhạc thôi là chưa đủ – Miho đã tìm đến Long Đỗ, đạo diễn trẻ nổi bật với phong cách hình ảnh tối giản và đầy chiều sâu. Miho Chu chia sẻ rằng anh chính là người chủ động hẹn gặp đạo diễn Long Đỗ để đề nghị hợp tác. Điều khiến Miho tin tưởng Long không chỉ là bởi những lần hợp tác trước đó, mà còn là gu thẩm mỹ và tư duy hình ảnh độc đáo mà Long thể hiện.</p>

      <p>Về phần mình, Long Đỗ cho biết anh đồng ý ngay bởi cảm nhận được "overview đầy tâm huyết" từ Miho – và hơn hết, anh thấy mình được tự do sáng tạo trong dự án lần này. Thế là một cú bắt tay nghệ thuật của hai nghệ sĩ Gen Z chính thức bắt đầu.</p>

      <p ><strong>"Playlist" – Một hành trình ký ức kể bằng âm nhạc và hình ảnh</p>

      <figure >
        <img src="./asset/bao/6/2.webp" alt="Hình ảnh từ MV Playlist" />
        <figcaption>Hình ảnh từ MV "Playlist"</figcaption>
      </figure>

      <p>Không đi theo cách kể chuyện tuyến tính, "Playlist" là những mảnh ký ức đan xen giữa hiện tại và quá khứ – nơi một nghệ sĩ trẻ trưởng thành cùng âm nhạc giữa gia đình, những khởi đầu vụng về và cả những khoảnh khắc muốn từ bỏ.</p>

      <p>Về mặt hình ảnh, Long Đỗ lựa chọn tone màu bụi, ánh sáng ngả vàng, máy quay tay và các cú lia máy chậm – tạo cảm giác thời gian đang trôi ngay trong khung hình. Theo đạo diễn, mỗi chi tiết trong MV đều có lý do tồn tại, không phải vì "thấy đẹp thì cho vào". Chính tư duy này khiến sản phẩm trở nên mạch lạc và đồng nhất giữa âm nhạc và hình ảnh.</p>

      <figure>
        <img src="./asset/bao/6/1.webp" alt="Hình ảnh từ MV Playlist" />
        <figcaption>Hình ảnh từ MV "Playlist"</figcaption>
      </figure>

      <p>"Playlist" cho thấy khi âm thanh và hình ảnh cùng đồng điệu, một câu chuyện cảm xúc có thể được kể mà không cần lời thoại. Đây là một sản phẩm nghệ thuật mang dấu ấn cá nhân rõ nét, đồng thời cũng là minh chứng cho khả năng kể chuyện bằng nghệ thuật của thế hệ nghệ sĩ trẻ. Sự kết hợp đầy triển vọng này đã mang đến một làn gió mới với không chỉ nền âm nhạc Việt Nam, mà còn cho thấy tương lai của nền nghệ thuật nước nhà dưới bàn tay của những nghệ sĩ trẻ.</p>

      <p style="text-align: right;"><strong>Biên tập: Đặng Ngân</strong></p>
      <div class="article-tags">
        <span class="article-tag">Spotlight</span>
      </div>
    `
  },

  article7: {
    category: "SAO",
    title: `Nghệ sĩ Thanh Thanh nói "Tôi sẽ ủng hộ sở thích của con cái dù có chuyện gì xảy ra"`,
    content: `
      <p class="time">Chủ nhật, 01/06/2025 | 10:00 (GMT+7)</p>
      <p class="lead">Trong buổi họp báo ra mắt MV "Playlist" của ca sĩ Miho Chu, nghệ sĩ Thanh Thanh – người đảm nhận vai người mẹ trong sản phẩm âm nhạc này – đã chia sẻ: "Tôi sẽ ủng hộ sở thích của con cái dù có chuyện gì xảy ra." Ở tuổi 42, nữ diễn viên không chỉ mang đến hình ảnh một người mẹ đầy trăn trở trên màn ảnh, mà còn cho thấy chính hành trình làm mẹ đầy thấu hiểu của mình sau cuộc hôn nhân đổ vỡ và hai cô con gái đang ở tuổi trưởng thành.</p>

      <p>Khi diễn viên Trần Thị Thanh Thanh tham gia đóng vai người mẹ trong MV "Playlist" của ca sĩ Chu Minh Hoàng (Miho Chu), cô đã tái hiện lại hành trình một người mẹ đi từ việc phản đối con cái theo đuổi đam mê âm nhạc tới sự thấu hiểu và ủng hộ con. Chính diễn viên Thanh Thanh chia sẻ, câu chuyện này có phần nào giống với câu chuyện thật của gia đình cô và chính cô cũng từng trải qua nhiều trăn trở khi đứng trước những cột mốc quan trọng của hai người con gái nhỏ.</p>

      <figure>
        <img src="./asset/bao/7/1.webp" alt="Diễn viên Thanh Thanh trong MV Playlist" />
        <figcaption>Diễn viên Thanh Thanh trong MV "Playlist"</figcaption>
      </figure>

      <p>Gần đây, diễn viên Thanh Thanh đã chính thức "đường ai nấy đi" với người chồng là doanh nhân thành đạt của mình. Theo những thông tin từ phía gia đình cô chia sẻ, hai cô con gái nhỏ của diễn viên Thanh Thanh sẽ sống cùng bố và lựa chọn tiếp nối sự nghiệp kinh doanh của bố thay vì theo đuổi con đường nghệ thuật như mẹ. Từ sau lùm xùm gia đình đổ vỡ, nữ nghệ sĩ ít khi nhắc tới chồng cũ và hai người con gái trước công chúng. Thế nhưng, trong buổi họp báo ra mắt MV "Playlist", diễn viên Thanh Thanh đã không ngần ngại chia sẻ thêm về câu chuyện gia đình của mình "Ai ở đây đã từng làm mẹ thì đều có những lựa chọn và đắn đo cho con của mình. Bản thân mình cũng từng lo lắng khi con cái quyết tâm theo đuổi sự nghiệp kinh doanh của bố. Thế nhưng sau cùng, mình vẫn ủng hộ con."</p>

      <p>Diễn viên cũng chia sẻ thêm, ở vai trò của người mẹ, cô hiểu rằng tình yêu của người mẹ không đơn thuần là sự quan tâm, lo lắng hay bắt ép con phải theo đuổi thứ mẹ cho là đúng nhất, tốt nhất. Diễn viên Thanh Thanh cho rằng chỉ cần mẹ nhìn được sự đam mê cháy bỏng của con thì không một người mẹ nào không hết mình ủng hộ con cái.</p>

      <figure>
        <img src="./asset/bao/7/2.webp" alt="Diễn viên Thanh Thanh chia sẻ câu chuyện làm mẹ" />
        <figcaption>Diễn viên Thanh Thanh chia sẻ câu chuyện làm mẹ</figcaption>
      </figure>

      <p>Dù cuộc hôn nhân không trọn vẹn, nữ diễn viên vẫn luôn trao trọn tình yêu thương và sự thấu hiểu sâu sắc cho hai cô con gái. Chính sự đồng hành đầy bao dung ấy có lẽ sẽ trở thành điểm tựa vững chắc, tiếp thêm động lực để các con vững bước trên hành trình phía trước.</p>

      <p style="text-align: right;"><strong>Biên tập: Sally Nguyễn</strong></p>
      <div class="article-tags">
        <span class="article-tag">Sao</span>
      </div>
    `
  },
  article8: {
    category: "ÂM NHẠC",
    title: `MV "Playlist": Câu chuyện về gia đình và giấc mơ nghệ thuật của ca sĩ Miho Chu`,
    content: `
      <p class="time">Chủ nhật, 02/06/2025 | 20:00 (GMT+7)</p>
      <p class="lead">MV "Playlist" là lời kể nhẹ nhàng của ca sĩ Miho Chu về tình yêu nghệ thuật và sự thấu hiểu trong gia đình, những điều mà ai cũng có thể bắt gặp trong hành trình theo đuổi ước mơ.</p>

      <figure>
        <img src="./asset/bao/8/1.webp" alt="Hình ảnh từ MV Playlist" />
        <figcaption>Hình ảnh từ MV "Playlist"</figcaption>
      </figure>

      <p>Ra mắt MV "Playlist" ngay trước thềm tháng gia đình, ca sĩ Miho Chu đã tái hiện một câu chuyện đầy cảm động về vai trò của gia đình trong hành trình nuôi dưỡng và theo đuổi đam mê âm nhạc của một cậu bé.</p>

      <p>Ca sĩ Miho Chu hóa thân thành người con trong một gia đình, nơi người cha là người theo đuổi âm nhạc và đã thầm lặng nuôi dưỡng đam mê ấy cho con mình. Tuổi thơ của cậu bé tràn ngập những giai điệu từ tiếng đàn của bố, được truyền cảm hứng bởi ước mơ nghệ thuật chưa trọn vẹn của người cha. Tuy nhiên, ước mơ ấy lại gặp phải sự phản đối từ người mẹ, người lo lắng về một tương lai bấp bênh và đầy thử thách. Cuối MV, cậu bé đã mạnh mẽ đứng trên sân khấu, viết tiếp câu chuyện âm nhạc của riêng mình, đồng thời nối dài hành trình đam mê mà bố đã bắt đầu dưới ánh mắt dõi theo và sự ủng hộ đầy yêu thương của mẹ.</p>

      <figure>
        <img src="./asset/bao/8/2.webp" alt="Khung cảnh gia đình trong MV Playlist" />
        <figcaption>Khung cảnh gia đình trong MV "Playlist"</figcaption>
      </figure>

      <p>MV được tạo nên từ những thước phim xen kẽ giữa hiện tại và quá khứ, dệt nên một mạch chuyện hoài niệm đầy sâu sắc. Đặc biệt, câu chuyện còn được kết nối khéo léo qua hình ảnh chú chim hạc giấy – biểu tượng của tuổi thơ rực rỡ và niềm vui giản dị. Mỗi lần xuất hiện, chú chim hạc như nhắc nhớ về những khoảnh khắc ngây thơ, những giấc mơ âm nhạc bắt đầu được ươm mầm trong vòng tay gia đình và niềm tin vào tương lai tươi sáng.</p>

      <p>Qua "Playlist", ca sĩ Miho Chu mang đến một câu chuyện gia đình đầy cảm xúc, nơi tình yêu thương và sự thấu hiểu dần lớn lên cùng hành trình theo đuổi đam mê của người con. MV như lời nhắn nhủ: gia đình luôn là nơi chắp cánh cho ước mơ bay xa.</p>

    
        <figure >
          <img src="./asset/bao/8/3.webp" alt="Chi tiết chim hạc giấy gắn với giấc mơ âm nhạc của cậu bé" />
            <figcaption>Chi tiết chim hạc giấy gắn với giấc mơ âm nhạc của cậu bé</figcaption>
        </figure>
        <figure >
          <img src="./asset/bao/8/4.webp" alt="Chim hạc giấy gắn liền với những ngày tháng tuổi thơ bên âm nhạc của bố" />
            <figcaption>Chim hạc giấy - biểu tượng xuyên suốt của tuổi thơ và ước mơ trong MV</figcaption>
        </figure>
      
    

      <p>"Playlist" đánh dấu sự trở lại của Miho Chu – ca sĩ trẻ từng được biết đến với hình ảnh ca sĩ nhí đầy triển vọng. Sau một quãng thời gian vắng bóng để trưởng thành và định hình phong cách cá nhân, ca sĩ Miho Chu tái xuất với một sản phẩm mang nhiều cảm xúc và chiều sâu hơn trong tư duy nghệ thuật.</p>

      <p>Không chỉ là một sản phẩm âm nhạc, Playlist còn là cách ca sĩ Miho Chu kể lại một lát cắt cuộc đời mình – nơi tuổi thơ gắn liền với âm nhạc, những ước mơ được ươm mầm trong tình yêu thương gia đình, và cả những mâu thuẫn, giằng xé nội tâm trên hành trình theo đuổi đam mê. Ca khúc mang màu sắc indie-pop hoài niệm, kết hợp giữa giai điệu nhẹ nhàng và không khí cảm xúc, như một hồi ức dịu dàng về tuổi thơ và ước mơ từng được chắp cánh trong vòng tay người thân.</p>

      <figure>
        <img src="./asset/bao/8/5.webp" alt="Phân cảnh tâm lý của cậu bé trong MV Playlist" />
        <figcaption>Phân cảnh tâm lý của cậu bé trong MV "Playlist"</figcaption>
      </figure>

      <p>MV còn cho thấy sự trưởng thành trong cách ca sĩ Miho Chu kể chuyện bằng âm nhạc. Sự mộc mạc trong ca từ, cộng hưởng cùng không khí gần gũi của hình ảnh, đã làm nên một bản nhạc vừa đủ sâu lắng để chạm đến ký ức, vừa đủ chân thành để khơi gợi đồng cảm.</p>

      <p>Trong nhịp sống hiện đại đầy hối hả, khi khoảng cách giữa các thế hệ trong gia đình đôi khi trở nên xa hơn bởi những khác biệt và áp lực cuộc sống, Playlist như một lời nhắc nhở nhẹ nhàng về sự thấu hiểu và yêu thương giữa cha mẹ và con cái. MV không chỉ là một bản nhạc cảm xúc, mà còn là món quà mà ca sĩ Miho Chu gửi đến khán giả nhân dịp tháng gia đình – một lời thì thầm rằng, dù hành trình trưởng thành có nhiều thử thách, tình yêu của gia đình vẫn luôn là nơi bắt đầu, và cũng là nơi để trở về.</p>

      <p style="text-align: right;"><strong>Biên tập: Đặng Ngân</strong></p>
      <div class="article-tags">
        <span class="article-tag">Âm nhạc</span>
      </div>
    `
  },

  article9: {
    category: "HẬU TRƯỜNG",
    title: `"Ca sĩ Miho Chu tự huy động nguồn lực sản xuất MV "Playlist" đánh dấu sự trở lại"`,
    content: `
      <p class="time">Thứ hai, 02/06/2025 | 09:00 (GMT+7)</p>
      <p class="lead">Ca sĩ Miho Chu ra mắt MV đầu tay "Playlist" đánh dấu sự trở lại sau thời gian dài rời khỏi ánh đèn sân khấu, nhưng ít ai biết đằng sau đó là cả một hành trình nỗ lực tự thân, khi nam ca sĩ trẻ gần như tự mình xoay xở để hoàn thiện từng khung hình bằng tất cả tình yêu với âm nhạc.</p>

      <figure>
        <img src="./asset/bao/9/1.webp" alt="Ca sĩ Miho Chu trong quá trình sản xuất MV" />
       
      </figure>

      <p>Sau 10 năm kể từ thời điểm nổi lên với tư cách là ca sĩ nhí, ca sĩ Miho Chu đã quyết định ra mắt sản phẩm âm nhạc đầu tay "Playlist" để quay trở lại với khán giả. Tuy nhiên đằng sau màn quay trở lại đặc biệt này của ca sĩ Miho Chu, nam ca sĩ trẻ đã phải tự mình đứng ra huy động nhiều nguồn lực thân quen – từ ekip, diễn viên cho đến đạo cụ.</p>

      <p>Đạo diễn Long Đỗ – người đứng sau hình ảnh của MV – cũng là một người bạn đồng trang lứa, mới 19 tuổi, đầy triển vọng và nhiệt huyết. Cả hai đã cùng nhau đồng hành trong quá trình làm việc, chia sẻ không chỉ góc nhìn nghệ thuật mà cả áp lực của những nghệ sĩ trẻ đang tìm chỗ đứng đầu tiên. Đạo diễn Long Đỗ cũng đã giúp ca sĩ Miho Chu kết nối với những diễn viên, ekip thân thiết sẵn sàng tham gia vào sản phẩm của Miho như diễn viên Quang Tiến (vai người bố), giám đốc nghệ thuật Nguyễn Ngọc Đoan Trang,...</p>

      <figure>
        <img src="./asset/bao/9/2.webp" alt="Ca sĩ Miho Chu chụp ảnh thân thiết cùng đội ngũ sản xuất MV" />
        <figcaption>Ca sĩ Miho Chu chụp ảnh thân thiết cùng đội ngũ sản xuất MV "Playlist"</figcaption>
      </figure>

      <p>Một điểm sáng đặc biệt trong MV là sự xuất hiện của diễn viên gạo cội Trần Thị Thanh Thanh trong vai người mẹ. Diễn viên Thanh Thanh đến với dự án như một mối duyên không hẹn trước, sau lần gặp tình cờ đạo diễn Long Đỗ tại một quán cà phê. Sự đồng cảm với tinh thần của những người trẻ nghiêm túc làm nghề khiến chị nhận lời tham gia, như một cách âm thầm tiếp sức cho thế hệ kế cận. Diễn viên Thanh Thanh chia sẻ chị nhìn thấy được tiềm năng và tâm huyết từ nghệ sĩ trẻ như Chu Minh Hoàng - những đam mê, nhiệt huyết mà theo cô: "...dù làm nghề lâu năm nhưng có những khát khao tuổi trẻ không thể tìm lại được".</p>

      <p>Suốt quá trình quay, ca sĩ Miho Chu còn nhận được sự đồng hành bền bỉ từ người bạn thân từ thuở nhỏ – người luôn theo sát từng chi tiết nhỏ nhất để giúp cậu hoàn thành mọi việc. Từ diễn viên phụ đến những vật dụng trong MV đều được vận dụng từ người quen, bạn bè, để từng cảnh quay không chỉ tiết kiệm mà còn chứa đựng nhiều yêu thương và sự nâng đỡ thầm lặng.</p>

      <figure>
        <img src="./asset/bao/9/3.webp" alt="Ca sĩ Miho Chu hỗ trợ đội ngũ sản xuất" />
        <figcaption>Ca sĩ Miho Chu hỗ trợ đội ngũ sản xuất trong quá trình quay MV</figcaption>
      </figure>

      <p>Để chuẩn bị cho buổi họp báo ra mắt MV Playlist, ca sĩ Miho Chu không chỉ xuất hiện với vai trò nghệ sĩ chính mà còn đích thân tham gia vào quá trình chuẩn bị hậu trường. Từng chi tiết nhỏ trong không gian sự kiện đều được anh kiểm tra kỹ lưỡng, từ khâu sắp xếp bàn ghế đến việc hỗ trợ ekip hoàn thiện phần trình bày. Sự tận tâm và chu đáo ấy cho thấy tinh thần trách nhiệm và niềm trân trọng mà nam ca sĩ dành cho sản phẩm đánh dấu sự trở lại sau nhiều năm vắng bóng.</p>

     
        <figure>
          <img src="./asset/bao/9/4.webp" alt="Miho Chu tại buổi tổng duyệt họp báo" />
          <figcaption>Miho Chu tại buổi tổng duyệt họp báo ra mắt MV "Playlist"</figcaption>
        </figure>
        <figure>
          <img src="./asset/bao/9/5.webp" alt="Miho Chu tham gia sắp xếp bàn ghế" />
          <figcaption>Miho Chu tham gia sắp xếp bàn ghế cho buổi họp báo</figcaption>
        </figure>
      

      <p>"Playlist" vì thế không chỉ là một MV âm nhạc, mà còn là thành quả của rất nhiều bàn tay, trái tim và sự tin tưởng mà ca sĩ Miho Chu đã nhận được trên hành trình trở lại – một hành trình không dễ dàng, nhưng đầy ý nghĩa.</p>

      <p style="text-align: right;"><strong>Biên tập: Đặng Ngân</strong></p>
      <div class="article-tags">
        <span class="article-tag">Hậu trường</span>
      </div>
    `
  },
}

// Load article content based on stored ID
document.addEventListener("DOMContentLoaded", () => {
  const currentArticle = localStorage.getItem("currentArticle") || "article1"
  loadArticleContent(currentArticle)
})

function loadArticleContent(articleId) {
  const article = articles[articleId]
  if (article) {
    document.getElementById("articleTitle").textContent = article.title
    document.getElementById("articleContent").innerHTML = article.content

    // Render related articles
    const relatedContainer = document.querySelector(".articles-grid")
    relatedContainer.innerHTML = ""

    Object.entries(articles).forEach(([id, a]) => {
      if (id !== articleId && a) { // Check if article exists
        const html = `
    <div class="article-card" onclick="loadArticle('${id}')">
      <div class="article-thumbnail">
        <img src="${a.image || (a.content ? getFirstImage(a.content) : null) || './asset/placeholder.jpg'}" alt="${a.title || ''}" />
      </div>
      <div class="article-info">
        <h3 class="article-title">${a.title || ''}</h3>
        <p class="article-meta">
          ${a.category ? `<span class="article-category">${a.category}</span>` : ''}
          ${a.date ? `<span class="article-date">${a.date}</span>` : ''}
        </p>
      </div>
    </div>
    `
        relatedContainer.insertAdjacentHTML("beforeend", html)
      }
    })
  }
}


function loadArticle(articleId) {
  // Add fade out effect
  document.body.style.opacity = '0'
  document.body.style.transition = 'opacity 0.3s ease'

  setTimeout(() => {
    // Update content
    localStorage.setItem("currentArticle", articleId)
    loadArticleContent(articleId)
    window.scrollTo(0, 0)

    // Fade back in
    document.body.style.opacity = '1'

    // Update URL without reload
    const newUrl = `?article=${articleId}`
    window.history.pushState({ articleId }, '', newUrl)
  }, 300)
}

// Handle browser back/forward buttons
window.onpopstate = function (event) {
  if (event.state && event.state.articleId) {
    loadArticleContent(event.state.articleId)
    window.scrollTo(0, 0)
  }
}
function goBack() {
  window.history.back()
}

function getFirstImage(content) {
  if (!content) return null;
  const match = content.match(/<img.*?src=["'](.*?)["']/)
  return match ? match[1] : null
}